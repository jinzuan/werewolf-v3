...
export class RoomService {
  ...
  async create(request: CreateRoomRequest): Promise<RoomAccess> {
    ...
    if (
      ...
      typeof request.createRequestId !== 'string' ||
      !request.createRequestId.trim() ||
      !options ||
    ...
    const createRequestId = request.createRequestId.trim();
  ...
  private startAI(roomCode: string, autoRoom = false): void {
    ...
    const run = this.driveAI(roomCode, autoRoom).finally(() => {
