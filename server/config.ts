...
const normalizeServerUrl = (url: string): string => {
  ...
  try {
    ...
    if (pathname === '/v1') parsed.pathname = '/v1/chat/completions';
...
export function loadAIConfig(): ServerAIConfig {
  ...
  // Apply them even when the optional JSON file is absent; that is the normal
  // container/deployment path for the V3 server.
