...
test('a single human can prepare and start a mixed room into the player match view', async ({ page, v3 }) => {
  ...
  await expect(page).toHaveURL(/\/rooms\/[A-Z2-9]{6}\/(?:play|waiting)$/, { timeout: 60_000 });
  await expect(page.getByText(/身份确认|正在恢复房间状态|等待准备|开始游戏/).first()).toBeVisible({ timeout: 60_000 });
});
